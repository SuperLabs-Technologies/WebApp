...
namespace WebApp
{
    public struct JsonArrayBuilder : IDisposable
    {
        ...
        public JsonArrayBuilder Push(JSONReader value)
        {
            EnsureState();
            writer.WriteCommaIf(startOffset);
            writer.Write(value);
        ...
        public JsonArrayBuilder PushObject(IEnumerable<KeyValuePair<string, JSONReader>> members)
            => PushObjectOf(members, (objectBuilder, member) => objectBuilder.Put(member.Key, member.Value));
        ...
        public JsonArrayBuilder PushIf(bool condition, JSONReader value) => condition ? Push(value) : this;
