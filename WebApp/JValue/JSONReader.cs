...
namespace WebApp
{
    ...
    public partial struct JSONReader : IComparable<JSONReader>, IEquatable<JSONReader>
    {
        #region TypeCode
        ...
        public static readonly JSONReader Null = new JSONReader("null", false);
        public static readonly JSONReader True = new JSONReader(true);
        public static readonly JSONReader False = new JSONReader(false);
        public static readonly JSONReader EmptyString = new JSONReader("\"\"", false);
        public static readonly JSONReader EmptyArray = new JSONReader("[]", false);
        public static readonly JSONReader EmptyObject = new JSONReader("{}", false);
        #endregion
        ...
        #region Constructors
        public static JSONReader Parse(string source)
        {
            if (source != null)
            {
                var index = SkipWhiteSpaces(source);
                var endIndex = BackwardSkipWhiteSpaces(source, source.Length - 1) + 1;
                return new JSONReader(source, index, endIndex - index);
            }
            else
                return Null;
        }

        public JSONReader(bool value) : this(value ? TrueLiteral : FalseLiteral, false) { }
        public JSONReader(int value) : this(value.ToString(NumberFormatInfo.InvariantInfo), false) { }
        public JSONReader(long value) : this(value.ToString(NumberFormatInfo.InvariantInfo), false) { }
        public JSONReader(ulong value) : this(value.ToString(NumberFormatInfo.InvariantInfo), false) { }
        public JSONReader(float value) : this(value.ToString(NumberFormatInfo.InvariantInfo), false) { }
        public JSONReader(double value) : this(value.ToString(NumberFormatInfo.InvariantInfo), false) { }
        public JSONReader(decimal value) : this(value.ToString(NumberFormatInfo.InvariantInfo), false) { }
        ...
        public JSONReader(IEnumerable<JSONReader> array) : this(From(array)) { }
        public JSONReader(IEnumerable<KeyValuePair<string, JSONReader>> obj) : this(From(obj)) { }
        internal JSONReader(string source, int startIndex, int length)
        {
            this.source = source;
        ...
        private JSONReader(string source, bool _) : this(source, 0, source.Length) { }
        private JSONReader(JSONReader original) : this(original.source, original.startIndex, original.length) { }

        private static JSONReader From(IEnumerable<JSONReader> array)
        {
        ...
        private static JSONReader From(IEnumerable<KeyValuePair<string, JSONReader>> obj)
        {
        ...
        #region Indexer
        public JSONReader this[int index] => Get(index);
        public JSONReader this[string key] => Get(key);
        #endregion
        ...
        private JSONReader ConvertForNumberParsing()
        {
            var endIndex = startIndex + length - 1;
            for (var i = startIndex + 1; i < endIndex; i++)
            {
                if (source[i] == '\\')
                    return new JSONReader(ToUnescapedStringCore(), false);
            }

            return new JSONReader(source, startIndex + 1, length - 2);
        }

        public List<JSONReader> ToArray()
        {
            var result = new List<JSONReader>();
        ...
        public Dictionary<JSONReader, JSONReader> ToObject()
        {
            var result = new Dictionary<JSONReader, JSONReader>();
        ...
        #region Get
        private JSONReader Get(int index)
        {
        ...
        private JSONReader Get(string key)
        {
            if (Type == TypeCode.Object)
            {
                ...
                while (keyStart < endIndex)
                {
                    ...
                    if (EqualsKey(key, source, keyStart, keyEnd - keyStart - 2))
                        return new JSONReader(source, valueStart, valueEnd - valueStart);
        ...
        #region Enumeration
        public IEnumerable<JSONReader> Array()
        {
            if (Type == TypeCode.Array)
            {
                var endIndex = startIndex + length - 1;
                ...
                {
                    var valueEnd = SkipValue(valueStart);
                    yield return new JSONReader(source, valueStart, valueEnd - valueStart);
                    valueStart = SkipWhiteSpaces(valueEnd + 1);
                }
            }
        }

        public IEnumerable<KeyValuePair<int, JSONReader>> IndexedArray()
        {
            if (Type == TypeCode.Array)
            {
                var endIndex = startIndex + length - 1;
                ...
                {
                    var valueEnd = SkipValue(valueStart);
                    yield return new KeyValuePair<int, JSONReader>(index++, new JSONReader(source, valueStart, valueEnd - valueStart));
                    valueStart = SkipWhiteSpaces(valueEnd + 1);
                }
            }
        }

        public IEnumerable<KeyValuePair<JSONReader, JSONReader>> Object()
        {
            if (Type == TypeCode.Object)
            {
                var endIndex = startIndex + length - 1;
                ...
                while (keyStart < endIndex)
                {
                    ...
                    yield return new KeyValuePair<JSONReader, JSONReader>(
                        new JSONReader(source, keyStart, keyEnd - keyStart),
                        new JSONReader(source, valueStart, valueEnd - valueStart));
                    keyStart = SkipWhiteSpaces(valueEnd + 1);
                }
            }
        }
        ...
        private static void Serialize(TextWriter writer, JSONReader value, int indent, int depth)
        {
            switch (value.Type)
            {
                case TypeCode.Array:
        ...
        private static void Serialize(TextWriter writer, IEnumerable<JSONReader> value, int indent, int depth, bool multiline)
        {
            writer.Write('[');
        ...
        private static void Serialize(TextWriter writer, IEnumerable<KeyValuePair<JSONReader, JSONReader>> value, int indent, int depth, bool multiline)
        {
            writer.Write('{');
        ...
        public int CompareTo(JSONReader other) => Compare(this, other);
        public bool Equals(JSONReader other) => Equals(this, other);
        public override bool Equals(object obj) => obj is JSONReader other && Equals(other);
        ...
        public static int Compare(JSONReader left, JSONReader right)
        {
        ...
        public static bool Equals(JSONReader left, JSONReader right)
        {
            var leftType = left.Type;
            var rightType = right.Type;
            if (leftType == rightType)
        ...
        private static int CompareMember(KeyValuePair<JSONReader, JSONReader> x, KeyValuePair<JSONReader, JSONReader> y)
        {
        ...
        private static bool EqualsString(JSONReader a, JSONReader b)
            => SequenceEqual<char, CharEnumerator>(a.GetCharEnumerator(), b.GetCharEnumerator(), (x, y) => x == y);
        private static bool EqualsMember(KeyValuePair<JSONReader, JSONReader> x, KeyValuePair<JSONReader, JSONReader> y)
        ...
        #region Operators
        public static bool operator ==(JSONReader left, JSONReader right) => left.Equals(right);
        public static bool operator !=(JSONReader left, JSONReader right) => left.Equals(right) == false;
        public static bool operator <(JSONReader left, JSONReader right) => left.CompareTo(right) < 0;
        public static bool operator <=(JSONReader left, JSONReader right) => left.CompareTo(right) <= 0;
        public static bool operator >(JSONReader left, JSONReader right) => left.CompareTo(right) > 0;
        public static bool operator >=(JSONReader left, JSONReader right) => left.CompareTo(right) >= 0;
        #endregion
        ...
        public static implicit operator bool(JSONReader value) => value.ToBoolean();
        public static implicit operator int(JSONReader value) => value.ToInt32();
        public static implicit operator long(JSONReader value) => value.ToInt64();
        public static implicit operator float(JSONReader value) => value.ToSingle();
        public static implicit operator double(JSONReader value) => value.ToDouble();
        public static implicit operator decimal(JSONReader value) => value.ToDecimal();
        public static implicit operator string(JSONReader value) => value.ToUnescapedString();
        public static implicit operator JSONReader(bool value) => new JSONReader(value);
        public static implicit operator JSONReader(int value) => new JSONReader(value);
        public static implicit operator JSONReader(long value) => new JSONReader(value);
        public static implicit operator JSONReader(float value) => new JSONReader(value);
        public static implicit operator JSONReader(double value) => new JSONReader(value);
        public static implicit operator JSONReader(decimal value) => new JSONReader(value);
        public static implicit operator JSONReader(string value) => new JSONReader(value);
        #endregion
    }
}
